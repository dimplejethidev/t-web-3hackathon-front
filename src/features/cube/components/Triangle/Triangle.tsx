...
export const Triangle = ({
  ...
  useFrame((state, delta) => {
    ...
    } else if (type === PRIZE_OBJ_TYPE.cube) {
    }
    return;
  });
  ...
  return (
    ...
    >
      <tetrahedronGeometry args={[Math.sqrt(3) / 3, 0]} />
...
const _adjustPos = (position: Position, type: PRIZE_OBJ_TYPE): Position => {
  ...
  return {
    ...
    y: position.y + BASE_Y + 0.19 + FLOOR_HEIGHT / 2,
    z: position.z + BASE_POSITION + 0.1,
  };
};

const _adjustRot = (rotation: Rotation): Position => {
  return {
    x: toRadian(rotation.x + 125),
    y: toRadian(rotation.y + 45),
